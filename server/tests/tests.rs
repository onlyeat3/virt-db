...
pub fn is_pattern_match(pattern: &str, sql2: &str, dialect: &MySqlDialect) -> bool {
    ...
    let tokens1:Vec<Token> = tokens1.into_iter()
        .filter(|t|{
            return match t {
                Token::EOF => {false}
        ...
        .collect();
    let tokens2:Vec<Token> = tokens2.into_iter()
        .filter(|t|{
            return match t {
                Token::EOF => {false}
                Token::Whitespace(_) => {false}
...
fn test_matchs(){
    ...
    sqls.insert("select count(1) from article where channel_id = ?  and tenant_id = ?","select count(1) from article where channel_id = 312  and tenant_id = 1");
    ...
    sqls.insert("select count(1) from article","select count(1) from article");
    sqls.insert("select * from article order by article_id desc limit ?","select * from article order by article_id desc limit 100");
